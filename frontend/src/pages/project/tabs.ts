    ...
    <div class="relative w-[min(1200px,96vw)] max-h-[90vh] overflow-hidden rounded-xl bg-neutral-800 ring-2 ring-neutral-600 shadow-2xl text-gray-100 pop-modal">
      ...
      <div class="flex h-[calc(90vh-3rem)]">
        <aside class="w-64 shrink-0 p-4 border-r border-neutral-600 space-y-2">
          ...
          <button class="tp-cat w-full text-left px-3 py-2 rounded hover:bg-neutral-800/40 text-sm ring-2 ring-transparent" data-cat="boards">ボード</button>
...
function templateCard(type: TabTemplate, title: string, highlight = false): string {
  const ring = highlight ? 'ring-emerald-600' : 'ring-neutral-600'
  return `
    <button data-tpl="${type}" class="group block rounded-xl overflow-hidden ring-2 ${ring} hover:ring-emerald-600 transition pop-card btn-press">
  ...
  if (type === 'kanban') {
    ...
    return `
      <div class="absolute inset-0 p-4 grid grid-cols-4 gap-2">
        <div class="bg-neutral-900/90 ring-2 ring-neutral-600 rounded">
